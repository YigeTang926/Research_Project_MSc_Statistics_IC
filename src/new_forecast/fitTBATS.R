fitSpecificTBATS <- function(y, use.box.cox, use.beta, use.damping, seasonal.periods=NULL, k.vector=NULL, 
                            starting.params=NULL, x.nought=NULL, ar.coefs=NULL, ma.coefs=NULL, 
                            v=NULL, u=NULL, init.box.cox=NULL, bc.lower=0, bc.upper=1, biasadj=FALSE) {
  ...
  if (use.box.cox) {
    ...
    optim.like <- optim(
      ...
      p = p, q = q, v = v, u = u, tau = tau, bc.lower = bc.lower, bc.upper = bc.upper,
    ...
    if (length(param.vector$vect) > 1) {
      optim.like <- optim(par = param.vector$vect, fn = calcLikelihoodNOTransformedTBATS, method = "Nelder-Mead",
                          opt.env = opt.env, x.nought = x.nought, use.beta = use.beta, use.small.phi = use.damping,
                          seasonal.periods = seasonal.periods, param.control = param.vector$control, 
                          p = p, q = q, v = v, u = u, tau = tau, 
                          control = list(maxit = (100 * length(param.vector$vect) ^ 2), parscale = par.scale))
    } else {
      optim.like <- optim(par = param.vector$vect, fn = calcLikelihoodNOTransformedTBATS, method = "BFGS", 
                          opt.env = opt.env, x.nought = x.nought, use.beta = use.beta, use.small.phi = use.damping, 
                          seasonal.periods = seasonal.periods, param.control = param.vector$control, 
                          p = p, q = q, v = v, u = u, tau = tau, 
  ...
  # Fit the GARCH(u,v) model and calculate the AIC
  if (is.null(v)&is.null(u)) {
    ...
    spec <- rugarch::ugarchspec(mean.model = list(armaOrder = c(0,0), include.mean = FALSE), 
                                variance.model = list(model="sGARCH", garchOrder=c(v, u)), 
                                distribution.model = "norm")
    garch_fit <- rugarch::ugarchfit(spec = spec, data = e[1,])
    aic <- likelihood + 2 * (length(param.vector$vect) + nrow(x.nought) + 1 + v + u)
    # Get the coefficients
    mu = coef(garch_fit)[1]
    if (v != 0) {
      arch.coefs = coef(garch_fit)[2:(1 + v)]
    } else {
      arch.coefs = NULL
    }
    if (u != 0) {
      garch.coefs = coef(garch_fit)[(2 + v):(1 + v + u)]
    } else {
      garch.coefs = NULL
    }
  }
  ...
  model.for.output <- list(
    lambda = lambda, alpha = alpha, beta = beta.v, damping.parameter = small.phi, gamma.one.values = gamma.one.v, gamma.two.values = gamma.two.v, 
    ar.coefficients = ar.coefs, ma.coefficients = ma.coefs, mu = mu, arch.coefficients = arch.coefs, garch.coefficients = garch.coefs,
    likelihood = likelihood, optim.return.code = optim.like$convergence, variance = variance, AIC = aic, parameters = list(vect = optim.like$par, control = param.vector$control), seed.states = x.nought,
    fitted.values = fits, errors = e, x = fitted.values.and.errors$x, seasonal.periods = seasonal.periods, k.vector = k.vector, y = y, p = p, q = q, v = v, u = u
  )
...
calcLikelihoodTBATS <- function(param.vector, opt.env, use.beta, use.small.phi, seasonal.periods, param.control, p=0, q=0, v=NULL, u=NULL, tau=0, bc.lower=0, bc.upper=1) {
  ...
  # Fit the GARCH model and calculate the log-likelihood
  if ((is.null(v))&(is.null(u))) {
    log.likelihood <- n * log(sum(opt.env$e * opt.env$e)) + n * (1 + log(2*pi)- log(n)) - 2 * (box.cox.parameter - 1) * sum(log(opt.env$y))
  } else {
    if (is.null(v)) {
      v <- 0
    }
    ...
    spec <- rugarch::ugarchspec(mean.model = list(armaOrder = c(0,0), include.mean = FALSE), variance.model = list(model="sGARCH", garchOrder=c(u, v)), distribution.model = "norm")
    fit <- rugarch::ugarchfit(spec = spec, data = opt.env$e[1,])  
    log.likelihood <- -2 * likelihood(fit) - 2 * (box.cox.parameter - 1) * sum(log(opt.env$y))
  }
...
calcLikelihoodNOTransformedTBATS <- function(param.vector, opt.env, x.nought, use.beta, use.small.phi, seasonal.periods, param.control, p=0, q=0, v=NULL, u=NULL, tau=0) {
  ...
  # Fit the GARCH model and calculate the log-likelihood
  if ((is.null(v))&(is.null(u))) {
    log.likelihood <- n * log(sum(opt.env$e * opt.env$e)) + n * (1 + log(2*pi)- log(n))
  } else {
    if (is.null(v)) {
      v <- 0
    }
    ...
    spec <- rugarch::ugarchspec(mean.model = list(armaOrder = c(0,0), include.mean = FALSE), variance.model = list(model="sGARCH", garchOrder=c(v, u)), distribution.model = "norm")
    fit <- rugarch::ugarchfit(spec = spec, data = opt.env$e[1,])  
    log.likelihood <- -2 * likelihood(fit)
  }
