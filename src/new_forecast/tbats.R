...
tbats <- function(y, use.box.cox=NULL, use.trend=NULL, use.damped.trend=NULL,
                  seasonal.periods=NULL, use.arma.errors=TRUE, use.garch.errors=TRUE, use.parallel=length(y) > 1000, num.cores=2,
  ...
  # Fit a TBATS model with no seasonal components and no ARMA errors
  ...
  # Find the optimal k_i for each seasonal period
  ...
  # Decide whether to include seasonal components
  ...
  if (use.garch.errors) {
    ar.coefs <- best.model$ar.coefficients
    ma.coefs <- best.model$ma.coefficients
    for (order in list(c(1,1), c(1,0), c(0,1), c(2,1), c(1,2))) {
      v <- order[1]
      u <- order[2]
      new.model <- try(fitSpecificTBATS(y, use.box.cox = model.params[1], use.beta = model.params[2],
                      use.damping = model.params[3], seasonal.periods = seasonal.periods,
                      k.vector = k.vector, ar.coefs=ar.coefs, ma.coefs=ma.coefs, v=v, u=u,
                      init.box.cox=init.box.cox, bc.lower = bc.lower, bc.upper = bc.upper, biasadj = biasadj
                      ), silent = TRUE
                    )
      if (is.element("try-error", class(new.model))) {
...
print.tbats <- function(x, ...) {
  ...
  if (!is.null(x$arch.coefficients)) {
    cat("\n  ARCH coefficients: ")
    cat(round(x$arch.coefficients, 6))
  }
  if (!is.null(x$garch.coefficients)) {
    cat("\n  GARCH coefficients: ")
    cat(round(x$garch.coefficients, 6))
  }
  cat("\n")
  cat("\nSeed States:\n")
  print(x$seed.states)
