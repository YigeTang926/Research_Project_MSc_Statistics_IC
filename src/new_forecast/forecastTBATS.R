...
as.character.tbats <- function(x, ...) {
  ...
  # seasonal periods and k vectors
  ...
  name <- paste(name, " + GARCH(", sep = "")
  # garch order
  if (!is.null(x$arch.coefficients)) {
    name <- paste(name, length(x$arch.coefficients), sep = "")
  } else {
    name <- paste(name, "0", sep = "")
  }
  name <- paste(name, ",", sep = "")
  if (!is.null(x$garch.coefficients)) {
    name <- paste(name, length(x$garch.coefficients), sep = "")
  } else {
    name <- paste(name, "0", sep = "")
  }
  name <- paste(name, ")", sep = "")
