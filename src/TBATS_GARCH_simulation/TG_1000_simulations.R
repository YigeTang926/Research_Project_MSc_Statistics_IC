...
i_am('src/TBATS_GARCH_simulation/TG_1000_simulations.R')
...
set.seed(123)
iteration <- 1000
matrix <- matrix(NA, nrow=iteration, ncol=22)
for (i in 1:iteration) {
    ...
    # matrix
    matrix[i,] <- c(aic, parameters1, parameters2, parameters3)
    }, error = function(e) {
    })
}

# Remove unreasonable rows
all_unreasonable_rows <- apply(matrix[,8:22], 1, function(row) any(row > 10))
results <- matrix[!all_unreasonable_rows, ]
